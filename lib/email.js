...
/**
 * Send a per-client monthly report email.
 */
export async function sendClientMonthlyReport({ email, businessName, htmlContent, monthName }) {
  if (!email) return { success: false, error: "No email provided" };
  ...
  try {
    const { error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
      subject: `${businessName} — AI Visibility Report for ${monthName}`,
      html: htmlContent,
    });

    if (error) {
      console.error(`Client report email error for ${businessName}:`, error);
      return { success: false, error };
    }
    return { success: true };
  } catch (err) {
    console.error(`Send client report failed for ${businessName}:`, err);
    return { success: false, error: err };
  }
}
