...
export async function buildMonthlyReportData(startDate, endDate) {
  ...
  // Ensure endDate covers the full day (timestamptz comparison excludes the day otherwise)
  const endDateFilter = endDate.includes("T") ? endDate : `${endDate}T23:59:59.999Z`;
  ...
  for (const client of clients) {
    ...
    const { data: rawResults } = await supabase
      .from("prompt_results")
      .select("ai_model, was_mentioned, mention_rank, run_batch_id, run_number, sentiment, response_status")
      .eq("client_id", client.id)
      .gte("checked_at", startDate)
      .lte("checked_at", endDateFilter);

    // Exclude invalid responses (refusals, empty, too_short) from scoring
