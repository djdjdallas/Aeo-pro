...
export async function runAlertChecks() {
  ...
  for (const client of clients) {
    try {
      ...
      // Alert 2: Significant daily drop (>20 points)
      ...
      try {
        ...
        // This week's data
        const { data: thisWeekResults } = await supabase
          .from("prompt_results")
          .select("was_mentioned")
          .eq("client_id", client.id)
        ...
        // Last week's data
        const { data: lastWeekResults } = await supabase
          .from("prompt_results")
          .select("was_mentioned")
          .eq("client_id", client.id)
