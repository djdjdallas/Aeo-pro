...
/**
 * Grounding classification for each AI model.
 * "web-grounded" = model has access to live web data during generation.
 * "training-data" = model responds from its training corpus only.
 */
const MODEL_GROUNDING_TYPE = {
  claude: "training-data",
  chatgpt: "training-data",
  perplexity: "web-grounded",
  gemini: "web-grounded",
  copilot: "web-grounded",
  grok: "training-data",
  deepseek: "training-data",
  meta_ai: "training-data",
};
...
/**
 * Patterns indicating a model refused or couldn't answer the query.
 * Checked case-insensitively against the full response text.
 */
const REFUSAL_PATTERNS = [
  ...
  "my knowledge cutoff",
  "i cannot recommend specific",
  "i'm not familiar with specific businesses",
];

const MIN_VALID_RESPONSE_LENGTH = 80;

/**
 * Validate an AI response before scoring.
 * Returns { isValid, status, reason } where status is one of:
 *   "valid"     — response is usable for scoring
 *   "empty"     — null/undefined/empty string
 *   "too_short" — below MIN_VALID_RESPONSE_LENGTH characters
 *   "refusal"   — matches a known refusal pattern
 */
export function validateAIResponse(responseText) {
  if (!responseText || responseText.trim().length === 0) {
    return { isValid: false, status: "empty", reason: null };
  }
  if (responseText.length < MIN_VALID_RESPONSE_LENGTH) {
    return { isValid: false, status: "too_short", reason: null };
  }
  const lower = responseText.toLowerCase();
  for (const pattern of REFUSAL_PATTERNS) {
    if (lower.includes(pattern)) {
      return { isValid: false, status: "refusal", reason: pattern };
    }
  }
  return { isValid: true, status: "valid", reason: null };
}

/**
 * Normalize a name for fuzzy comparison: lowercase, strip punctuation,
 * collapse whitespace.
 * "Joe's Plumbing, LLC" → "joes plumbing llc"
 */
...
/**
 * Generate variant forms of a business name for fuzzy matching.
 * Returns a deduplicated array of lowercase strings to check against.
 */
export function generateNameVariants(businessName) {
  const lower = businessName.toLowerCase().trim();
  const normalized = normalizeName(businessName);
  const noSpaces = normalized.replace(/\s+/g, "");
  const hyphenated = normalized.replace(/\s+/g, "-");

  const variants = [lower, normalized, noSpaces, hyphenated];
  return [...new Set(variants)];
}

/**
 * Shared mention-detection logic used by all AI models.
 * Checks the primary business name, all generated variants, and any
 * explicit name_aliases provided by the admin.
 */
export function parseMentionFromResponse(fullResponse, businessName, targetUrl, nameAliases = []) {
  const lowerResponse = fullResponse.toLowerCase();
  const normalizedResponse = normalizeName(fullResponse);

  let wasMentioned = false;
  let mentionRank = null;
  let responseSnippet = null;

  // Build the full set of names to check: primary name + aliases
  const allNames = [businessName, ...nameAliases];
  ...
  for (const name of allNames) {
    ...
    for (const variant of variants) {
      if (!variant) continue;
      // Check the variant against both the lowered and the normalized response
      if (lowerResponse.includes(variant) || normalizedResponse.includes(variant)) {
        wasMentioned = true;

        // Compute rank from the original response using the lowered variant
        const sentences = fullResponse.split(/[.!?]+/);
        const mentionIndex = sentences.findIndex((s) => {
          const sLower = s.toLowerCase();
          const sNorm = normalizeName(s);
          return sLower.includes(variant) || sNorm.includes(variant);
        });
        mentionRank = mentionIndex + 1;

        // Build snippet using the first match position in lowerResponse
        const matchTarget = lowerResponse.includes(variant) ? lowerResponse : normalizedResponse;
        const mentionPos = matchTarget.indexOf(variant);
 ...
 */
async function checkPromptAgainstAI(prompt, businessName, targetUrl, model, nameAliases = []) {
  let fullResponse = "";
  let citations = [];
  ...
  const validation = validateAIResponse(fullResponse);
  ...
  if (!validation.isValid) {
    return {
      ...
      citations,
      grounding_type: MODEL_GROUNDING_TYPE[model] || "unknown",
  ...
  const { wasMentioned, mentionRank, responseSnippet } =
    parseMentionFromResponse(fullResponse, businessName, targetUrl, nameAliases);
...
async function processResult(supabase, result, prompt, client, clientId, batchId, shot, shotWindow) {
  const insertRow = {
    ...
    grounding_type: result.grounding_type || "unknown",
    response_status: result.response_status || "valid",
  };
  ...
  // Skip downstream processing for invalid responses
  if (result.skip_scoring) return;
  ...
  try {
    ...
    if (names.length > 0) {
      // Build normalized set of client names for is_client matching
      const clientNames = [client.business_name, ...(client.name_aliases || [])];
      const clientNormalized = new Set(clientNames.map((n) => normalizeName(n)));

      const mentionRows = names.map((name, idx) => ({
        result_id: inserted.id,
        client_id: clientId,
        business_name: name,
        is_client: clientNormalized.has(normalizeName(name)),
...
export async function runClientShot(clientId, shotNumber = 1, shotWindow = null) {
  ...
  const nameAliases = client.name_aliases || [];
  ...
  const validResults = allResults.filter((r) => r.response_status === "valid");

  return {
    checked: validResults.length,
    mentioned: validResults.filter((r) => r.was_mentioned).length,
    skipped_invalid: allResults.length - validResults.length,
    results: allResults,
    batchId,
  };
}
