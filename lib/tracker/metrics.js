...
/**
 * Returns true if the sample size is large enough for reliable scores.
 */
export function isScoreReliable(n) {
  return n >= MIN_RELIABLE_SAMPLE;
}

/**
 * Returns data collection progress info for a client.
 */
export function getDataCollectionProgress(n) {
  return {
    isReliable: n >= MIN_RELIABLE_SAMPLE,
    checksCompleted: n,
    checksNeeded: Math.max(0, MIN_RELIABLE_SAMPLE - n),
    progressPercent: Math.min(100, Math.round((n / MIN_RELIABLE_SAMPLE) * 100)),
  };
}
